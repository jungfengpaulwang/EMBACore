...
{
    public partial class Course_Brief_Export_DBF
    {
        private ExportProxyForm exportProxyForm;
        public Course_Brief_Export_DBF()
        {
        }
...
left join $ischool.emba.teacher_ext as te on teacher.id=te.ref_teacher_id where course.id in ({0}) order by course.id", String.Join(",", K12.Presentation.NLDPanels.Course.SelectedSource));
            ...
            exportProxyForm.Text = "匯出課程檔(DBF 格式)";
            exportProxyForm.lblExplanation.Text = "以下欄位將被寫入DBF檔";
        ...
        private void _BGWLoadData_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            try
            {
                ...
                if (result)
                {
                    ...
                    MessageBox.Show(message, "匯出課程檔(DBF 格式)", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, MessageBoxOptions.DefaultDesktopOnly);
                }
            }
            catch (Exception ex)
            {
